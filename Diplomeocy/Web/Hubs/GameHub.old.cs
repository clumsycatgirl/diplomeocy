...
public class GameHubOld : Hub {
	private readonly DatabaseContext databaseContext;
	private readonly Dictionary<string, GameHandler> gameHandlers;
	private readonly ILogger<GameHub> logger;

	public GameHubOld(DatabaseContext databaseContext, ILogger<GameHub> logger, Dictionary<string, GameHandler> gameHandlers) : base() {
	...
	public Task AddBauild(string gameId, string country, string location, string unitType) {
		if (!gameHandlers.TryGetValue(gameId, out GameHandler? handler)) {
			return Clients.Client(Context.ConnectionId).SendAsync("RequestError", $"invalid gameId: '{gameId}'");
		}
